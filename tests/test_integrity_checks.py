...
def test_uv_range_and_clear_sky():
    assert any(f.check_id == "uv_range" for f in check_uv([_h(uv_index=18.0)]))
    assert any(
        f.check_id == "uv_above_clear_sky"
        for f in check_uv_vs_clear_sky([_h(uv_index=9.0, uv_index_clear_sky=6.0)])
    )
...
def test_hi_below_air_temp_critical():
    # T > 80F and HI < T is a math break
    findings = check_hi_gte_air_temp([_h(temp_f=95.0, heat_index_f=90.0)])
    assert any(f.check_id == "hi_below_air_temp" and f.severity == Severity.CRITICAL for f in findings)
    assert check_hi_gte_air_temp([_h(temp_f=95.0, heat_index_f=98.0)]) == []


def test_dew_point_ok_and_bad():
...
def test_corrupted_feed_triggers_errors():
    ...
    hours = [
        _h(
            ...
            uv_index=7.0,
            uv_index_clear_sky=5.0,  # uv > clear sky
            heat_index_f=90.0,
            temp_f=95.0,  # HI < T when T>80
            pm2_5=-5.0,  # corrupt
            us_aqi=50.0,
        )
        for i in range(24)
    ...
    assert "hi_below_air_temp" in ids
    assert any(f.severity == Severity.CRITICAL for f in findings)
